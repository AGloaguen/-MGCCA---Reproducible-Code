...
rgccak <- function (A, C, tau = "optimal", scheme = "centroid", scale = FALSE, 
  ...
  repeat {
    ...
    stopping_criteria_tmp = sqrt(drop(crossprod(Reduce("c", a) - Reduce("c", a_old)))/drop(crossprod(Reduce("c", a_old))))
    stopping_criteria     = c(stopping_criteria_tmp, 
                              (crit[iter]-crit_old)/crit_old)
    
    if ( any(stopping_criteria < tol) | (iter > 1000))
    #if ( ( sum(unlist(lapply(mapply("-", a, a_old), function(x) drop(crossprod(x))))) < tol) | (iter > 1000) ) 
       break
  ...
  if(iter<1000 & verbose) cat("The RGCCA algorithm converged to a solution of the stationary equations after", iter-1, "iterations \n")
