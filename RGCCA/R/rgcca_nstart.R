...
rgcca_nstart <- function(A, C = 1-diag(length(A)), tau = rep(1, length(A)), ncomp = rep(1, length(A)), scheme = "centroid", scale = TRUE, scale_size_bloc = TRUE , init="svd", bias = TRUE, tol = 1e-8, verbose=TRUE, nstart_next_comps) {
  ...
  if (N == 0) {
    ...
    for (j in 1:J) AVE_X[[j]] =  4#mean(cor(A[[j]], Y[[j]])^2)
  ...
  for (n in 1:N) {
    ...
    if(n>1 && is.character(init)){
      for (start in 1:nstart_next_comps){
        if(is.vector(tau))
          tmp_rgcca.result <- rgccak(R, C, tau = tau , scheme=scheme, init = init, bias = bias, tol = tol, verbose=verbose) 
        else
          tmp_rgcca.result <- rgccak(R, C, tau = tau[n, ] , scheme=scheme, init = init, bias = bias, tol = tol, verbose=verbose) 
        if(start == 1){
          rgcca.result = tmp_rgcca.result
          max_crit     = max(rgcca.result$crit)
        }else{
          if(max(tmp_rgcca.result$crit) > max_crit){
            rgcca.result = tmp_rgcca.result
            max_crit     = max(rgcca.result$crit)
          }
        }
      ...
      if(is.vector(tau))
        rgcca.result <- rgccak(R, C, tau = tau , scheme=scheme, init = cur_init, bias = bias, tol = tol, verbose=verbose) 
      else
        rgcca.result <- rgccak(R, C, tau = tau[n, ] , scheme=scheme, init = cur_init, bias = bias, tol = tol, verbose=verbose) 
    }
  ...
  if(is.character(init)){
    for (start in 1:nstart_next_comps){
      if(is.vector(tau)) 
        ...
        tmp_rgcca.result <- rgccak(R, C, tau = tau[N+1, ] , scheme=scheme, init = init, bias = bias, tol = tol, verbose=verbose) 
      if(start == 1){
        rgcca.result = tmp_rgcca.result
        max_crit     = max(rgcca.result$crit)
      }else{
    ...
    cur_init = lapply(init, function(x) x[, N+1])
    if(is.vector(tau)) 
      tmp_rgcca.result <- rgccak(R, C, tau = tau , scheme=scheme, init = cur_init, bias = bias, tol = tol, verbose=verbose) 
    else
      tmp_rgcca.result <- rgccak(R, C, tau = tau[N+1, ] , scheme=scheme, init = cur_init, bias = bias, tol = tol, verbose=verbose) 
  }
  ...
  #Average Variance Explained (AVE) per block
  for (j in 1:J) AVE_X[[j]] =  4#apply(cor(A[[j]], Y[[j]])^2, 2, mean)
