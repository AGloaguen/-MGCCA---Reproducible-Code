...
sgccak <-  function(A, C, c1 = rep(1, length(A)), scheme = "centroid", scale = FALSE,
  ...
  #  Choose the J outer weight vectors a_j by svd or random initialisation
  if (init=="svd") {
  ...
  #	Apply the L1 and L2 norm constraints on a_j
  ...
  repeat{
      ...
      for (q in 1:J){
        ...
        if (mode(scheme) == "function") {
          assign(formalArgs(scheme), cov2(Y[, q], Y, bias = bias))
          CbyCovq = C[q,]*attr(eval(dg), "grad")
        } 
    ...
    if ( any(stopping_criteria < tol) | (iter > 100)) 
        break
  ...
  if (iter > 100) warning("The SGCCA algorithm did not converge after 1000 iterations.")
  if(iter<100 & verbose) cat("The SGCCA algorithm converged to a solution of the stationary equations after", iter-1, "iterations \n") 
