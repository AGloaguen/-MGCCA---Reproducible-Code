mgccak_array_KKT <- function (A, A_m = NULL, C, tau = "optimal", scheme = "centroid", center = FALSE, scale = FALSE, 
                    verbose = FALSE, init="svd", bias = TRUE, tol = 1e-8, M_J = NULL, M_K = NULL, Proj_J = NULL, Proj_K = NULL,
                    M_regularisation, nb_iter) {
  ...
  a <- b <- c <- M_reg <- M_inv <- P <- tau_l <- Q <-list()
  ...
  repeat {
    for (j in 1:L){
      ...
      #Tensors
      if (j %in% B_3D){
        Q[[j]] = matrix(t(Z[, j]) %*% P[[j]], nrow = Kls[j], ncol = Jls[j], byrow = T)
        SVD    = svd(x = Q[[j]], nu = 1, nv = 1)
        c[[j]] = SVD$u
        b[[j]] = SVD$v
        Y[, j] = P[[j]] %*% (c[[j]] %x% b[[j]])
      }else{
    ...
    if (iter > 1){
      ...
      partial_derivate = lapply(1:L, function(x) c(Q[[x]] %*% b[[x]],t(Q[[x]]) %*% c[[x]]))
      partial_derivate = matrix(Reduce("c", partial_derivate), ncol = 1)
      diag_a           = mapply(function(x, y) c(2*drop(crossprod(y))*x, 2*drop(crossprod(x))*y), c, b)
      diag_a           = as.matrix(bdiag(lapply(1:L, function(x) matrix(diag_a[[x]], ncol = 1) )))
      proj             = diag_a %*% ginv(t(diag_a) %*% diag_a, tol = .Machine$double.eps) %*% t(diag_a)
      kkt              = c(kkt, sqrt(crossprod((diag((dim(proj)[1])) - proj) %*% partial_derivate))/sqrt(crossprod(partial_derivate)))

      #Stop if all criterion are satisfied
      if (iter > nb_iter) break
    }    
