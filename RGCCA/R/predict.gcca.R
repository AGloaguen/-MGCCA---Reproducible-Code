...
predict.gcca = function (object, newA, type = c("regression", "classification","coefficient"), fit = c("lm", "cor", "lda", "logistic"), 
  ...
  if (type != "coefficient") {
    if (missing(newA)) {
      ...
      if (length(names(A)) != B &&  length(names(newA)) != newB) stop("Please, number of blocs and number of blocs named are not the same")
