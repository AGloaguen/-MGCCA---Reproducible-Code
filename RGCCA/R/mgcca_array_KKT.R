mgcca_array_KKT <- function(A, C = 1-diag(length(A)), tau = rep(1, length(A)), ncomp = rep(1, length(A)), scheme = "centroid", center = TRUE, scale = TRUE , 
                        init="svd", bias = TRUE, tol = 1e-8, verbose=TRUE, method_scale = "3-way", M_J = NULL, M_K = NULL, deflation_mode = NULL,
                        M_regularisation, nstart = 1, nstart_at_comp_2 = F, nb_iter) {
  ...
  for (n in 1:(N+1)) {
    ...
    for (start in 1:nstart_comp){
      #MGCCA algorithm
      tmp_mgcca.result = mgccak_array_KKT(A          = R, 
                                          A_m        = R_m, 
                                          C          = C, 
                                          tau        = tau[n, ], 
                                          scheme     = scheme, 
